...
using UnityEditor;
    ...
    public class SerializePropertyAttributeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (this.propertyFieldInfo == null)
                this.propertyFieldInfo = target.GetType().GetProperty(((SerializeProperty)attribute).PropertyName,
                                                     BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 
            if (this.propertyFieldInfo != null){

                  // Retrieve the value using the property getter:
                object value = this.propertyFieldInfo.GetValue(target,null);
 
                // Draw the property, checking for changes:
                EditorGUI.BeginChangeCheck();
                value = this.DrawProperty(position,property.propertyType,this.propertyFieldInfo.PropertyType,value,label);
     
                // If any changes were detected, call the property setter:
                if (EditorGUI.EndChangeCheck() && this.propertyFieldInfo != null){
     
                    // Record object state for undo:
                    Undo.RecordObject(target, "Inspector");
     
                    // Call property setter:
                    this.propertyFieldInfo.SetValue(target,value,null);
                }
